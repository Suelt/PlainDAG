...
func (fm *FRoundMsg) commitWithEmbeded(n *Node) error {
	...
	for i := fm.LeastEmbededRn; i < fm.GetRN(); i++ {
		...
		for _, msg := range msgs {
			...
			if len(msg.GetPlainMsgs().Txs) != 0 {
				log.Println("commit message at round", i, "from node", id, "commit latency:", time.Now().UnixMilli()-msg.GetTimestamp())
				log.Println("transactions contained:", len(msg.GetPlainMsgs().Txs))
			}
		}
...
func (fm *FRoundMsg) GetPlainMsgs() ttype.BatchTx {
