...
func (m *BasicMsg) GetSource() string {
	return m.Source
}

func (m *BasicMsg) GetInteger() []int {
	return m.Integer
}
...
func (m *BasicMsg) GetPlainMsgs() ttype.BatchTx {
	return m.Plainmsg
}

func NewBasicMsg(rn int, refs [][]byte, source string, batchtx ttype.BatchTx) (*BasicMsg, error) {
	m := BasicMsg{
		Rn:         rn,
		References: refs,
		Source:     source,
...
func GenesisBasicMsg(rn int, refs [][]byte, source string) (*BasicMsg, error) {

	//r := rand.New(rand.NewSource(time.Now().UnixNano()))
	//z := zipf.NewZipf(r, skew, 10000)
	batchtx := ttype.BatchTx{

		Txs: make([]*ttype.Transaction, 1),
	}
